...
> **Disclaimer:** Not a single line of code in this repository was written by a
> human — every line was generated by an AI assistant and only reviewed by a
> human.
...
- Upload only accumulates when at least one leecher is in the swarm
- Optional ratio cap (`--client.max-ratio`) stops counting once target is hit
- Per-tick jitter on upload rate avoids suspiciously smooth curves
...
1. Turn auth on with `--auth.enabled` and set `--http.public-url` to the
...
| Command | Description |
|---------|-------------|
| `emission seed` | CLI-only watcher — no API, no auth, no sidecar |
...
```
--storage.torrents      directory to watch (default: ~/.local/share/emission/torrents)
--client.name           client profile to impersonate (default: transmission-4.0.6)
--client.min-speed      minimum upload rate (default 50K)
--client.max-speed      maximum upload rate (default 500K)
--client.max-peers      peers to request per tracker (0 = client default)
--client.max-ratio      stop accumulating upload at N × torrent size (0 = unlimited)
--log-level             log verbosity: trace, debug, info, warn, error (default info)
--config                config file path (auto-discovered if unset)
```

### `serve`-only flags

```
--storage.auth          passkey credential file (default: ~/.local/share/emission/auth.json)
--auth.enabled          require passkey authentication
--http.api              serve the JSON API
--http.ui               serve the web UI (implies --http.api)
--http.port             HTTP port (default 8080)
--http.tls.enabled      serve over HTTPS
--http.tls.cert         TLS certificate PEM (required with --http.tls.enabled)
--http.tls.key          TLS private key PEM (required with --http.tls.enabled)
--http.public-url       externally reachable base URL (required with --auth.enabled)
```
...
PATCH  /api/torrents/{id}      update per-torrent overrides {minSpeed, maxSpeed, maxRatio}
...
POST   /api/auth/invite        create invite link (admin only)
...                            full spec at /docs
```

The WebSocket pushes `{type:"stats",torrents:[…]}` ~once per second and
`{type:"changed"}` whenever the torrent list changes.
...
```sh
emission serve --http.ui --auth.enabled --http.public-url https://your.host
```
...
- **Inviting other users:** the admin clicks "Invite" in the UI, enters
  a username, gets a one-time link (and a 3-word code). Send the link
  to the user; they open it and register their own device. Codes expire
  in 24 hours.
...
- Go **1.24+** (uses module-level `tool` directive for `swag`)
- Node **20+** and npm (the build embeds the UI into the binary)
- `make`
