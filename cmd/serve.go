...
accepted.`,
	...
	cmd.Flags().Bool("auth.enabled", false, "require passkey authentication for the HTTP API/UI")
...
// setupAuth builds the auth service when --auth.enabled is on. Returns nil
...
func setupAuth(apiEnabled bool) (*auth.Service, error) {
	...
	if !apiEnabled {
		return nil, fmt.Errorf("--auth.enabled needs --http.api or --http.ui")
	}
	publicURL := viper.GetString("http.public-url")
	if publicURL == "" {
		return nil, fmt.Errorf("--auth.enabled requires --http.public-url")
	}
