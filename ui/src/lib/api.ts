...
/** removeMyDevice deletes one of the current user's own passkeys. */
export async function removeMyDevice(id: string): Promise<void> {
  await request(`/auth/me/devices/${encodeURIComponent(id)}`, { method: 'DELETE' })
}

/** deleteMyAccount removes the current user's account and all their data. */
...
export interface PendingInvite {
  token: string
  username: string
  createdBy?: string // who minted this invite
  expiresAt: number // unix ms
}

/** listInvites returns all non-expired pending invite tokens (admin only). */
export async function listInvites(): Promise<PendingInvite[]> {
  const res = await request('/auth/invites')
  return (await res.json()) as PendingInvite[]
}

/** revokeInvite deletes a pending invite by token (admin only). */
export async function revokeInvite(token: string): Promise<void> {
  await request(`/auth/invites/${encodeURIComponent(token)}`, { method: 'DELETE' })
}

export interface StatsPoint {
  t: number // unix milliseconds
  r: number // rate bytes/sec
  l: number // leechers
}

/** getTorrentStats fetches the historical rate/leecher stats for one torrent. */
export async function getTorrentStats(id: string): Promise<StatsPoint[]> {
  const res = await request(`/torrents/${encodeURIComponent(id)}/stats`)
  return (await res.json()) as StatsPoint[]
}
...
export interface StatsHandlers {
  ...
  onChanged: () => void
  /** onConnected fires when the WebSocket connects or reconnects. */
  onConnected?: () => void
  /** onDisconnected fires when the WebSocket drops (before retry). */
  onDisconnected?: () => void
  /** onStatsHistory receives the full stats buffer for all torrents on connect. */
  onStatsHistory?: (history: Record<string, StatsPoint[]>) => void
  /** onStatPoint receives one new data point for a single torrent (~every 30 s). */
  onStatPoint?: (id: string, point: StatsPoint) => void
}
...
export function connectStats(handlers: StatsHandlers): () => void {
  ...
  const open = () => {
    ...
    socket.onopen = () => handlers.onConnected?.()
    socket.onmessage = (e) => {
      try {
        const msg = JSON.parse(e.data as string) as {
          type: string
          torrents?: Torrent[]
          history?: Record<string, StatsPoint[]>
          id?: string
          point?: StatsPoint
        }
        if (msg.type === 'stats') handlers.onStats(msg.torrents ?? [])
        else if (msg.type === 'changed') handlers.onChanged()
        else if (msg.type === 'stats_history' && msg.history) handlers.onStatsHistory?.(msg.history)
        else if (msg.type === 'stat_point' && msg.id && msg.point) handlers.onStatPoint?.(msg.id, msg.point)
      } catch {
