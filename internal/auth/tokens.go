...
// Create mints a new invite token enrolling the given username. The token is a
// 3-word code (e.g. "arrogant-jimmy-dumpster") — easy to read aloud.
func (s *InviteStore) Create(username string) (string, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	for attempt := 0; attempt < 8; attempt++ {
		...
		s.tokens[tok] = inviteEntry{username: username, expiry: time.Now().Add(s.ttl)}
		return tok, nil
	}
...
// Consume validates a token, removes it, and returns its username. A token can
// be used at most once. ok is false for an unknown or expired token.
func (s *InviteStore) Consume(token string) (username string, ok bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	e, ok := s.tokens[token]
	if !ok {
