...
// is still open: no credentials exist yet, and the server started less than
// bootstrapWindow ago.
func (s *Service) BootstrapOpen() bool {
	return s.creds.Count() == 0 && time.Since(s.startedAt) < bootstrapWindow
}
...
// An empty username makes a bootstrap invite (the registrant names themselves).
func (s *Service) CreateInvite(username string) (string, error) {
	return s.invites.Create(username)
}
...
func (s *Service) BeginRegistration(invite string) (options *protocol.CredentialCreation, ceremonyID, username string, err error) {
	...
	// Exclude every already-registered credential so one authenticator cannot
	// enrol twice — one physical device backs exactly one passkey/user.
	exclude := webauthn.Credentials(s.creds.WebAuthnCredentials()).CredentialDescriptors()
	options, sessionData, err := s.wa.BeginRegistration(s.creds, webauthn.WithExclusions(exclude))
	if err != nil {
		return nil, "", "", err
	}
...
func (s *Service) FinishRegistration(ceremonyID string, r *http.Request) (string, error) {
	...
	if c.invite == "" {
		...
		username = AdminUsername
	} else if username, ok = s.invites.Consume(c.invite); !ok {
	...
	if err := s.creds.Add(*cred, username); err != nil {
		return "", err
	}
...
func (s *Service) BeginLogin() (*protocol.CredentialAssertion, string, error) {
	...
	options, sessionData, err := s.wa.BeginLogin(s.creds)
	if err != nil {
		return nil, "", err
	}
...
func (s *Service) FinishLogin(ceremonyID string, r *http.Request) (string, error) {
	...
	cred, err := s.wa.FinishLogin(s.creds, c.data, r)
	if err != nil {
		return "", err
	}
