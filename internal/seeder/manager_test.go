...
func TestManagerAddFileUsesSidecar(t *testing.T) {
	srv := newTrackerServer(t)
	dir := t.TempDir()
	m := newTestManager(t, dir)
	path := newTestTorrent(t, dir, "x", srv.URL)

	// Pre-write a sidecar with values different from the AddFile args.
	if err := SaveSidecar(path, 100, 1024, 1.5); err != nil {
		t.Fatal(err)
	}
	...
	if st.MinRateBytesPerSec != 100 || st.MaxRateBytesPerSec != 1024 || st.MaxRatio != 1.5 {
		t.Errorf("sidecar not applied: %+v", st)
	}
}
...
func TestManagerSetClientOptions(t *testing.T) {
	...
	if err := m.SetClientOptions(st.ID, 200, 1024, 2.0); err != nil {
		t.Fatal(err)
	}
	...
	// SetClientOptions persists to sidecar.
	min, max, ratio, ok := LoadSidecar(path)
	if !ok || min != 200 || max != 1024 || ratio != 2.0 {
		t.Errorf("sidecar = (%d, %d, %v, %v)", min, max, ratio, ok)
	}
}
...
func TestManagerSetClientOptionsValidation(t *testing.T) {
	...
	if err := m.SetClientOptions(st.ID, 500, 100, 0); err == nil {
		t.Error("min > max should error")
	}
	if err := m.SetClientOptions(st.ID, 100, 500, -1); err == nil {
		t.Error("negative ratio should error")
	}
	if err := m.SetClientOptions("deadbeef", 100, 500, 0); err == nil {
